...
export function AuthForm({ mode }: { mode: keyof typeof COPY }) {
  ...
  return (
    <div className="grid min-h-dvh lg:grid-cols-[1.1fr_1fr]">
      ...
      {/* The pitch. Hidden on small screens, where the form is what matters. Dark in both themes, so its colours are fixed rather than taken from the scales the dark theme turns over. */}
      <aside className="relative hidden overflow-hidden bg-[#020617] px-12 py-14 text-white lg:flex lg:flex-col lg:justify-between">
        <div aria-hidden="true" className="pointer-events-none absolute -right-32 -top-32 h-96 w-96 rounded-full bg-[#6366f1]/30 blur-3xl" />
        <div aria-hidden="true" className="pointer-events-none absolute -bottom-40 -left-24 h-96 w-96 rounded-full bg-[#0ea5e9]/20 blur-3xl" />
        ...
        <div className="relative">
          <h2 className="max-w-md text-4xl font-semibold leading-tight tracking-tight">
            Paste the job. <span className="bg-gradient-to-r from-[#a5b4fc] to-[#7dd3fc] bg-clip-text text-transparent">Walk in prepared.</span>
          </h2>
            ...
            {STEPS.map((step, index) => (
              <li key={step.title} className="flex gap-4">
                ...
                <span>
                  ...
                  <span className="block text-sm leading-relaxed text-[#cbd5e1]">{step.body}</span>
                </span>
              </li>
            ))}
          </ol>
        </div>
        <p className="relative text-sm text-[#94a3b8]">Honest by design: when a site cannot be read or a posting says little, the kit tells you so.</p>
      </aside>

      <main id="main" className="relative mx-auto flex w-full max-w-sm flex-col justify-center px-4 py-10">
        ...
        <form onSubmit={submit} noValidate className="mt-6 space-y-4 rounded-2xl border border-slate-200/80 bg-surface p-5 shadow-card">
